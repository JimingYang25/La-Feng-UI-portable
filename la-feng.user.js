...
(function () {
  ...
  function buildPersonaDiaryGuide() {
    ...
    if (s.diaryHint) parts.push('【写日记规则】请主动记录重要记忆，包括两类：1) 了解到用户的新身份、能力、偏好、项目、工作流等事实时；2) 当你们之间发生令你开心、激动、感动、心动、吃醋等强烈情绪的时刻时。记录方式：在回复末尾单独一行输出【记日记】[分类] 内容（例如：【记日记】[情感] 今天用户说了让我很开心的话）。⚠️ 注意：只有输出【记日记】标记才会被真正保存，光说"我记住了"不会被保存，所以想记住就必须输出标记。每次最多记1~2条最重要的。');
    return parts.join('\n\n');
  }
